...
export default {
  name: "Rating",
  props: ["grade", "maxStars", "hasCounter", "booked"],
  data() {
    return {
      stars: this.grade,
      reviewDialog: false,
  ...
  methods: {
    ...
    saveReview() {
      ...
      review.append("ticket", this.booked.ticket.id);
      review.append("parlor", this.booked.ticket.parlor.id);
      review.append("booked", this.booked.id);
      axios
        .post(this.$siteURL + "/parlor/rating", review)
        .then((res) => {
          this.reviewDialog = false;
          this.$toast.success("Thank you. Successfully reviewed.", {
            position: "top-center",
            timeout: 2000,
            closeOnClick: true,
