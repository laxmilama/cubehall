<template>
  <div>
    <span
      class="btn-secondary btn-normalize follow-button"
      v-if="isFollowing"
      @click="showUnfollow"
      ref="flg"
    >
    ...
    <span
      :style="btnColor()"
      class="btn-normalize follow-button"
      v-show="!isFollowing"
      @click="follow()"
      >Follow</span
    >
    ...
    >
      <div style="text-align: center">
        ...
        <section class="_unFlw_msg">
          Are you sure you want to unfollow @{{ studio.slug }}?
        </section>
...
export default {
  ...
  methods: {
    ...
    unfollow() {
      axios
        .delete(this.$siteURL + "/unfollow/studio/" + this.studio.id)
        .then((res) => {
    ...
    btnColor() {
      let [back, txt] = Color.GetButtonColor(this.studio.brand_color);
      // console.log(back, txt);
      return {
        background: back,
        color: txt,
      };
