<template>
  <div class="parlor-card">
    <div class="explore-parlor-list" style="width: 100%">
      <div v-if="!available" class="parlor-sold">sold out</div>
      <div class="parlor-card-box round_c_s">
        <img
          class="parlor-card-box-img"
          :src="$siteURL + '/images/parlor/small/' + thumbnail"
          width="100%"
        />
      </div>
      ...
      <a
        class="parlor-link"
        :href="`${$siteURL}/parlor/${slug}?c=${toBash(parlor, type, parent)}`"
        data-collector
      ></a>
    </div>
  </div>
