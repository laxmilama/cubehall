  ...
  <div v-if="creators.length > 0" style="display: flex; flex-direction: column">
    <div class="DC_studio_card" v-for="(creator, i) in creators" :key="i">
      ...
      <div class="DC_studio_body">
        <div class="DC_studio-box" v-for="(thumb, j) in creator.showoffs" :key="j">
          <img
            :src="`${$siteURL}/images/showoff/thumb/` + thumb.thumbnail"
            class="DC_studio-profile-img"
          />
        </div>
      </div>
...
<script>
import FollowUserBasic from "./FollowUserBasic.vue";
export default {
  name: "PeopleDiscovery",
  components: { FollowUserBasic },
  data() {
    return {
      creators: [],
      data: null,
  ...
  methods: {
    ...
    getResults() {
      ...
      axios
        .get(this.$siteURL + "/get/people/discover?page=" + this.page)
        .then((res) => {
