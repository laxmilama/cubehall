...
class ProductSimilarity
{
    ...
    public function __construct(array $products)
    {
        ...
        $this->priceHighRange = max(array_column($products, 'user_id'));
    }
    ...
    {
        $productAFeatures = $productA->description;
        $productBFeatures = $productB->description;

        return array_sum([
            (Similarity::hamming($productAFeatures, $productBFeatures) * $this->featureWeight),
            (Similarity::euclidean(
                Similarity::minMaxNorm([$productA->user_id], 0, $this->priceHighRange),
                Similarity::minMaxNorm([$productB->user_id], 0, $this->priceHighRange)
            ) * $this->priceWeight),
            (Similarity::jaccard($productA->title, $productB->title) * $this->categoryWeight)
